...
#define CIPHER_DEFAULT_CIPHER "aes-128-cbc"
...
public:
  ...
  typedef uchar aes_key_t[16];
  typedef uchar aes_iv_t[16];
...
public:
   ...
   *      // % openssl enc -e -a -md sha256 -aes-256-cbc -salt -p \
...
public:
   ...
   *      // Decrypt file.
   *      // It is the same as running this command:
   *      // % openssl enc -d -a -md sha256 -aes-256-cbc -salt -p \
   *      //       -pass pass:password                          \
   ...
   * @endcode
   *
   * Note that binary salt values are not allowed by the
   * interface. That is not an internal restriction. It
   * would be a simple matter to add support if it was
   * requested.
   *
...
//  std::string file_read(const std::string& fn) const;
   ...
   * @param nl    Append a trailing new line.
...
//  void set_salt(const std::string& salt);
  ...
  bool set_iv(const std::string& iv);
