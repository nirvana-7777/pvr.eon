...
output_root="${addon_root}/build/docker-linux-aarch64"
image_tag="pvr-eon-builder:linux-aarch64"
...
docker build \
  ...
  -t "${image_tag}" \
  -f "${script_dir}/linux-aarch64.Dockerfile" \
  "${addon_root}"
...
docker run --rm \
  ...
  -e LINUX_HOST="${LINUX_HOST:-aarch64-linux-gnu}" \
